...
7. **Resolve `consumes` parameters first.** If any of the chosen shortcut's parameters declare `consumes: {from_id, key}`, fill them by calling `registry.chain_state({op: "get", from_id, key, expected_type})` BEFORE executing actions. The call throws if the producing shortcut hasn't run yet — recover by running the producer first (look it up in the registry, execute, verify, then set its produces value as in step 10). Treat the returned `value` as the parameter's runtime value; substitute it into action templates the same way you would a user-supplied parameter.
