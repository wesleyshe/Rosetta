...
- [x] ~~Manual test (Claude Desktop) U1~~: subsumed by Phase 7a Part 3 Photoshop smoke (2026-05-04) — the use-skill protocol's core path (lookup → os_action → verify → registry_report_execution) ran end-to-end against the live Railway backend and produced a Wilson-bounded ShortcutStats row.
- [x] ~~Manual test (Claude Desktop) U2~~ (fallback path): subsumed by Phase 7b#2 — `verify(interpret_check)` was refactored agent-side, eliminating the ROSETTA_ANTHROPIC_API_KEY dependency. The 7b/9b/9c branches in use.md fire correctly.
- [x] ~~Manual test (Claude Desktop) E1~~ (offline lifecycle): subsumed by Phase 7b#3 (2026-05-04) — explorer-mode submitted 5 verified shortcuts on the live backend; full lifecycle (start_session → save_finding → submit_findings) validated against real GitHub PR auto-merge.
- [x] ~~Manual test (resume)~~: protocol-level resume already verified in Phase 4 smoke #4; live verification subsumed by E1 above.
- [x] ~~Iterate on prompt wording~~: superseded by Phase 7b#4 explore-skill prompt fix (payment_destination, all five metadata sub-fields).
